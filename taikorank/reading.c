...
static int tro_mesh_has_volume(const struct tr_object * o)
{
    int has = 1;
    if (!gts_surface_is_closed(o->mesh)) {
	tr_error("Mesh is not closed!");
	has = 0;
    }
    if (!gts_surface_is_manifold(o->mesh)) {
	tr_error("Mesh is not manifold!");
	has = 0;
    }
    if (!gts_surface_is_orientable(o->mesh)) {
...
#define GTS_SURFACE_CLONE(surf)				\
    GTS_SURFACE(gts_object_clone(GTS_OBJECT(surf)))

/*
  Should remove others superposition but not working for now
 */
/*
static void tro_inter(const struct tr_object * o)
{
    GtsSurface * mesh = GTS_SURFACE_CLONE(o->mesh);
    for (int i = 0; i < table_len(o->obj_h); i++) {
	const struct tr_object * o2 = table_get(o->obj_h, 0);
	if (o2 == NULL) // already done with same bpm
	    continue;
	GtsSurface * mesh2 = GTS_SURFACE_CLONE(o2->mesh);
	GtsSurface * tmp = tr_gts_exclude_2_to_1(mesh, mesh2);
	gts_object_destroy(GTS_OBJECT(mesh));
	mesh = tmp;
    }
}
*/
...
void tro_set_app_dis_offset(struct tr_object * o)
{
    ...
    if (o->offset_app < map->oect[0].offset - OFFSET_MIN)
    {
    o->offset_app = map->oect[0].offset - OFFSET_MIN;
    o->offset_dis = o->end_offset       + OFFSET_MAX;
    }
    */
    ...
    /*
      if(tro_is_slider(o))
      {
      int diff = o->end_offset - o->offset;
      o->end_offset_app = o->offset_app + diff;
      o->end_offset_dis = o->offset_dis + diff;
      }
      else if(o->type == 's')
      {
      int diff = o->end_offset - o->offset;
