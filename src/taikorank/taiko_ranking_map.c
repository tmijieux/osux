...
static int get_tro_type_from_osux_ho(struct hit_object * ho);
static double get_bpm_app_from_osux_tp(struct timing_point * tp,
				       double sv);
static int get_tro_end_offset_from_osux_ho(struct hit_object * ho,
...
static struct tr_map * trm_from_osux_map(struct osux_beatmap * map);
...
static double get_bpm_app_from_osux_tp(struct timing_point * tp, double sv)
{
...
static struct tr_map * trm_from_file(char *filename)
{
    ...
    return trm_from_osux_map(map);
}

//---------------------------------------------------------------

static struct osux_beatmap * osux_map_check_mode(struct osux_beatmap *map)
{
    switch(map->Mode) {
    case MODE_STD:
	if (OPT_AUTOCONVERT) {
...
static struct tr_map * trm_from_osux_map(struct osux_beatmap * map)
{
	...
	struct hit_object * ho   = &map->HitObjects[i];
	struct timing_point * tp = &map->TimingPoints[current_tp];
