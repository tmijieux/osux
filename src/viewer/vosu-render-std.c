...
Bezier_de_Casteljau(uint32_t n, // control point count = n+1
                    double t, // t € [ 0, 1 ]
                    dvec2 W[n+1]) // control points
{
    int s = n+1;
    dvec2 *X = ARRAY_DUP(W, s);
    dvec2 *Y = g_malloc(sizeof*Y * s);
    dvec2 *tmp, p;
